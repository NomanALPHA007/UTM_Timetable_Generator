...
class CourseTimeItem extends React.PureComponent {
  ...
  componentDidMount() {
    this.timeItemHTML = document.getElementById(
      `course-item__times__entry-${this.props.courseID}-${this.props.timeInfo.timeID}`
    );
  ...
  render() {
    ...
    return (
      <div
        className="course-item__times__entry"
        id={`course-item__times__entry-${courseID}-${timeID}`}
      >
        <button
          type="button"
          className="course-item__times__entry__delete-button"
        ...
        <select
          className="course-item__times__entry__dayOfWeek"
          value={dayOfWeek}
          onChange={this.changeTimeInfo}
        >
          <option value="0">
            {dayOfWeekIndexConverter(0, isIslamicWeekend, true)}
          </option>
          <option value="1">
            {dayOfWeekIndexConverter(1, isIslamicWeekend, true)}
          </option>
          <option value="2">
            {dayOfWeekIndexConverter(2, isIslamicWeekend, true)}
          </option>
          <option value="3">
            {dayOfWeekIndexConverter(3, isIslamicWeekend, true)}
          </option>
          <option value="4">
            {dayOfWeekIndexConverter(4, isIslamicWeekend, true)}
          </option>
        </select>

        <select
          className="course-item__times__entry__session"
          value={session}
          onChange={this.changeTimeInfo}
        >
